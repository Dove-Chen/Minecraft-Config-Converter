...
from src.migrators.to_oraxen import ToOraxenMigrator


class IAToOraxenConverter(BaseConverter):
    def __init__(self):
        super().__init__()
        self.oraxen_config = {
            "items": {},
    ...
    def set_resource_paths(self, ia_root, oraxen_pack_root):
    ...
    def save_config(self, output_root):
        ...
        recipes_dir = os.path.join(output_root, "recipes")
        os.makedirs(items_dir, exist_ok=True)

        if self.oraxen_config["items"]:
            self._write_yaml_with_footer(
                self.oraxen_config["items"],
                os.path.join(items_dir, f"{self.namespace}.yml"),
            )

        if self.oraxen_config["categories"]:
            self._write_yaml_with_footer(
                {"categories": self.oraxen_config["categories"]},
                os.path.join(items_dir, f"{self.namespace}_categories.yml"),
            )

        if self.oraxen_config["recipes"]:
            grouped = {}
            for recipe_id, recipe_data in self.oraxen_config["recipes"].items():
                if not isinstance(recipe_data, dict):
                    continue
                recipe_type = str(recipe_data.get("type", "shaped")).lower()
                entry = dict(recipe_data)
                entry.pop("type", None)
                if recipe_type == "shaped" and "pattern" in entry and "shape" not in entry:
        ...
        if self.ia_resourcepack_root and self.oraxen_pack_root:
            ToOraxenMigrator(
                self.ia_resourcepack_root,
                self.oraxen_pack_root,
                self.namespace,
            ).migrate()
    ...
    def _convert_items(self, items):
        ...
        for raw_id, item_data in items.items():
            ...
            converted = self._convert_item(item_id, item_data)
            if converted:
                self.oraxen_config["items"][item_id] = converted
    ...
    def _convert_item(self, item_id, item_data):
        ...
        oraxen_item = {"material": material}

        enabled = self._get_dict_value(item_data, "enabled")
        if enabled is not None:
            oraxen_item["enabled"] = bool(enabled)

        name = self._get_item_name(item_data)
        if name:
            oraxen_item["displayname"] = name

        lore = self._normalize_lore(self._get_dict_value(item_data, "lore"))
        if lore:
            oraxen_item["lore"] = lore

        permission = self._get_dict_value(item_data, "permission", "permission_suffix", "permission-suffix")
        if isinstance(permission, str) and permission.strip():
            oraxen_item["permission"] = permission

        enchants = self._get_dict_value(item_data, "enchants", "enchantments")
        if enchants:
            oraxen_item["enchants"] = enchants

        model_id = self._get_dict_value(resource, "model_id", "custom_model_data", "custom-model-data")
        if model_id is not None:
            oraxen_item["model"] = model_id

        self._apply_pack(oraxen_item, resource, item_data, slot)
        self._apply_components(oraxen_item, item_data, slot)
        self._apply_mechanics(oraxen_item, item_data)
        self._apply_attribute_modifiers(oraxen_item, item_data)
        self._apply_durability(oraxen_item, item_data)
    ...
    def _apply_pack(self, oraxen_item, resource, item_data, slot):
        pack = {}
        ...
        model_path = self._normalize_resource_path(self._get_dict_value(resource, "model_path", "model"))
        if model_path:
        ...
        if normal_texture:
            pack["textures"] = [self._to_resource_ref(normal_texture, suffix_to_strip=".png")]
        elif textures:
            pack["textures"] = textures
        self._apply_pack_state_models(pack, resource, str(oraxen_item.get("material", "")).upper())
        ...
        if pack:
            oraxen_item["Pack"] = pack

    def _apply_components(self, oraxen_item, item_data, slot):
        ...
        if explicit_slot:
            oraxen_item.setdefault("Components", {}).setdefault("equippable", {})["slot"] = explicit_slot

    def _apply_mechanics(self, oraxen_item, item_data):
        behaviours = self._get_dict_value(item_data, "behaviours", "behaviors", default={})
        if not isinstance(behaviours, dict):
            return
        ...
        if mechanics:
            oraxen_item["Mechanics"] = mechanics

    def _apply_attribute_modifiers(self, oraxen_item, item_data):
        ...
        if result:
            oraxen_item["AttributeModifiers"] = result

    def _apply_durability(self, oraxen_item, item_data):
        ...
        if value is not None:
            oraxen_item.setdefault("Mechanics", {}).setdefault("durability", {})["value"] = value
    ...
    def _convert_categories(self, categories):
        ...
        for raw_id, category_data in categories.items():
            ...
            enabled = self._get_dict_value(category_data, "enabled")
            if enabled is not None:
                entry["enabled"] = bool(enabled)
            permission = self._get_dict_value(category_data, "permission")
            if isinstance(permission, str) and permission.strip():
                entry["permission"] = permission
            self.oraxen_config["categories"][category_id] = entry
        ...
        for group_key, group_data in recipes.items():
            ...
            for raw_id, recipe_data in group_data.items():
                ...
                for recipe_id, converted in self._convert_recipe_entry(group, raw_id, recipe_data):
                    if converted:
                        self.oraxen_config["recipes"][recipe_id] = converted
    ...
    def _convert_recipe_result(self, result):
        ...
        if item_id is None:
            return None
        converted = {"item": self._normalize_recipe_value(item_id)}
    ...
    def _normalize_recipe_value(self, value):
        ...
        if isinstance(value, str):
            return self._normalize_oraxen_recipe_item(value)
        return value

    def _normalize_oraxen_recipe_item(self, value):
        if not isinstance(value, str) or not value.strip():
            return value
        ref = value.strip()
        if ref.startswith("#"):
    ...
    def _extract_recipe_patterns(self, recipe_data):
        patterns = []
        base_pattern = self._get_dict_value(recipe_data, "pattern", "shape")
        if base_pattern:
